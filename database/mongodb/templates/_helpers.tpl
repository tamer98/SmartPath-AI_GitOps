...
{{/*
Return the proper MCP MongoDB image name
*/}}
{{- define "mongodb.mcp.image" -}}
{{- include "common.images.image" (dict "imageRoot" .Values.mcp.image "global" .Values.global) -}}
{{- end -}}

{{/*
Return the proper kube-rbac-proxy image name
*/}}
{{- define "mongodb.mcp.auth.image" -}}
{{- include "common.images.image" (dict "imageRoot" .Values.mcp.auth.image "global" .Values.global) -}}
{{- end -}}

{{/*
Return the proper Docker Image Registry Secret Names for MCP
*/}}
{{- define "mongodb.mcp.imagePullSecrets" -}}
{{- include "common.images.renderPullSecrets" (dict "images" (list .Values.mcp.image .Values.mcp.auth.image) "context" $) -}}
{{- end }}

{{/*
Returns the proper service account name for MCP depending if an explicit service account name is set
in the values file. If the name is not set it will default to either common.names.fullname if mcp.serviceAccount.create
is true or default otherwise.
*/}}
{{- define "mongodb.mcp.serviceAccountName" -}}
    {{- if .Values.mcp.serviceAccount.create -}}
        {{ default (printf "%s-mcp" (include "common.names.fullname" .)) .Values.mcp.serviceAccount.name }}
    {{- else -}}
...
{{/*
Return the MCP MongoDB fullname
*/}}
{{- define "mongodb.mcp.fullname" -}}
{{- printf "%s-mcp" (include "common.names.fullname" .) -}}
{{- end -}}

{{/*
Return the MCP auth TLS secret name
*/}}
{{- define "mongodb.mcp.auth.tls.secretName" -}}
{{- if or .Values.mcp.auth.tls.autoGenerated.enabled (and (not (empty .Values.mcp.auth.tls.cert)) (not (empty .Values.mcp.auth.tls.key))) -}}
    {{- printf "%s-mcp-auth-crt" (include "common.names.fullname" .) -}}
{{- else -}}
    {{- required "An existing secret name must be provided with TLS certs for MCP auth if cert and key are not provided!" (tpl .Values.mcp.auth.tls.existingSecret .) -}}
{{- end -}}
{{- end -}}
